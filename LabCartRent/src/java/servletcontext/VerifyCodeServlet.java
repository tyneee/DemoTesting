...
public class VerifyCodeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (txtCode.trim().equals(code.trim())) {

                TblRegistrationDAO dao = new TblRegistrationDAO();
                boolean result = dao.updateStatusAccount(email);
                if (result) {
                    url = map.get("verifyPageSuccess");
                }
            } else {
                request.setAttribute("errorCode", "Invalid verify code");
            }
